...
//|                                                                  |
//+------------------------------------------------------------------+
class PredictionsApplier3Labels
  {
protected:
      struct output
      {
...
void PredictionsApplier3Labels::Processing()
{
   ...
   */
   ...
   if (prediction_id > -1) {
      ...
      if (outputs[prediction_id].signal == 2) {
         // If position is open, profitable, and signal is sell, sell entire position
         if (PositionSelect(Symbol())) {
            m_trade.PositionClose(Symbol());
         }
      }
      else if (outputs[prediction_id].signal == 1) {
         if (outputs[prediction_id].confidence > 0) {
...
void PredictionsApplier3Labels::UpdateTrailingStops()
{
